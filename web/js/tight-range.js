/* Tight Range — strategy page logic.
   Loads daily bars for the selected instrument, finds every quiet stretch whose
   trailing close-to-close range sat inside a tight band around its average,
   measures what happened over the next 1 / 5 / 10 days, and renders the
   scoreboard + event log. A range can break either way — there is no direction
   control. All computation is local to the browser; data is pre-built JSON
   served from web/data/. */
...
const state = {
  instrument: "spx",
  band:       3,        // half-width of the band, in percent
  window:     10,       // trailing trading days the range is measured over
  entry:      "close",
  range:      "5y",
};
...
function renderControlsState(){
  ...
  document.getElementById("band-val").textContent = `±${state.band.toFixed(1)}%`;
  document.getElementById("band-range").value = state.band;

  document.getElementById("window-val").textContent = `${state.window} DAYS`;
  document.getElementById("window-range").value = state.window;
}
...
function renderSummary(stats){
  const rows = [
    { label:"WINS",        sub:s => `out of ${s.n} quiet stretches`, get:s => `${s.wins} / ${s.n}`, color:() => "" },
    { label:"WIN RATE",    sub:() => "how often it worked",  get:s => `${fmtInt(s.rate)}%`, color:s => s.rate >= 50 ? "pos" : "neg" },
    { label:"AVG RETURN",  sub:() => "per stretch",          get:s => fmt(s.avg),           color:s => s.avg > 0 ? "pos" : "neg" },
    { label:"MEDIAN",      sub:() => "the typical stretch",  get:s => fmt(s.med),           color:s => s.med > 0 ? "pos" : "neg" },
...
function renderLog(events){
  ...
  if(events.length === 0){
    body.innerHTML = `<tr class="empty"><td colspan="5">No quiet stretches match these settings. Widen the band, shorten the range length, or widen the date range.</td></tr>`;
    count.textContent = "0 stretches";
    return;
  }
  body.innerHTML = [...events].reverse().map(e =>
    `<tr><td>${e.date}</td>` +
    `<td class='trig'>${e.trig.toFixed(1)}%</td>` +
    `<td class='${cls(e.d1)}'>${fmt(e.d1)}</td>` +
    `<td class='${cls(e.d2)}'>${fmt(e.d2)}</td>` +
    `<td class='${cls(e.d3)}'>${fmt(e.d3)}</td></tr>`
  ).join("");
  count.textContent = `${events.length} stretch${events.length === 1 ? "" : "es"} found`;
}
...
async function update(){
  ...
  const events  = findRangeEvents(payload.bars, state);
...
function wireControls(){
  ...
  for(const segId of ["entry-seg","range-seg"]){
  ...
  document.getElementById("band-range").addEventListener("input", e => {
    state.band = parseFloat(e.target.value);
    renderControlsState();
  });
  document.getElementById("band-range").addEventListener("change", update);

  document.getElementById("window-range").addEventListener("input", e => {
    state.window = parseInt(e.target.value, 10);
    renderControlsState();
  });
  document.getElementById("window-range").addEventListener("change", update);
}
