/* Moving-Average Cross — strategy page logic.
   Loads daily bars for the selected instrument, finds every time the close
   crossed its N-day average in the chosen direction, measures what happened
   over the next 5 / 10 / 20 days, and renders the scoreboard + event log. All
   computation is local to the browser; data is pre-built JSON served from
   web/data/. */
...
const state = {
  instrument: "spx",
  direction:  "up",     // 'up' = crossed above, 'down' = crossed below
  period:     200,      // trailing trading days the average is measured over
  entry:      "close",
  range:      "5y",
};
...
function renderLog(events){
  ...
  if(events.length === 0){
    body.innerHTML = `<tr class="empty"><td colspan="5">No crosses match these settings. Try the other direction, shorten the average, or widen the date range.</td></tr>`;
