/* Multi-Day Move — strategy page logic.
   Loads daily bars for the selected instrument, finds every stretch whose
   trailing N-day close-to-close move cleared the threshold in the chosen
   direction, measures what happened over the next 1 / 5 / 10 days, and renders
   the scoreboard + event log. All computation is local to the browser; data is
   pre-built JSON served from web/data/. */
...
const state = {
  instrument: "spx",
  direction:  "down",   // 'down' = sell-off, 'up' = run-up
  threshold:  5.0,      // absolute percent; sign comes from direction
  window:     5,        // trailing trading days the move is measured over
  entry:      "close",
  range:      "5y",
};
...
function renderControlsState(){
  ...
  document.getElementById("track-low").textContent  = `${sign}2%`;
  document.getElementById("track-high").textContent = `${sign}20%`;
...
function renderLog(events){
  ...
  if(events.length === 0){
    body.innerHTML = `<tr class="empty"><td colspan="5">No events match these settings. Loosen the threshold, shorten the window, or widen the date range.</td></tr>`;
...
async function update(){
  ...
  const events  = findMultiDayEvents(payload.bars, state);
