/* Tight Range — league table page logic.
   Loads all instrument bars in parallel, computes per-instrument tight-range
   stats from the same shared settings, renders a sortable comparison table.
   Re-uses the strategy-engine so numbers match the single-instrument page. The
   d1/d2/d3 stat slots map to the 1 / 5 / 10 day horizons; this table surfaces
   the day-1 and day-10 columns. A range can break either way — there is no
   direction control. */
...
const state = {
  ...
  entry:     "close",
  range:     "5y",
  sortKey:   "avg_d3",   // avg over the 10-day horizon
  sortDir:   "desc",
};
...
function renderTable(){
  ...
  document.getElementById("league-note").textContent =
    `${rows.length} instruments · ${totalEvents} total stretches`;
