...
> **Version note (rev. 2, July 2026).** Revised after an independent three-analyst
> review, then finalised with the account holder's decisions. Rev. 1 fixed the
> *honesty of the framing* — the downside is now stated as the model actually
> reports it, the "hedge" and "diversifier" claims are qualified, and the glidepath
> is made explicit. Rev. 2 records the **two investment decisions** the review
> surfaced: **silver (`ISLN.L`) is dropped and replaced with water (`IH2O.L`)** in
> the rotating sleeve, and the **~34% Avantis weighting is consciously kept**; the
> lump sum is being **invested in full at once**. The core is unchanged. The review
> and exactly what was and wasn't actioned are in the companion note
> [`isa_5y_analyst_review.md`](isa_5y_analyst_review.md); the one-page build sheet is
> [`isa_5y_build_sheet.md`](isa_5y_build_sheet.md).
>
> *Numbers note:* the headline risk/return figures below are from the silver-in
> Monte-Carlo build. Water has a **higher expected return (~6% vs ~0.8% compounded)
> and lower volatility (~15% vs ~28%)** than the silver it replaces, so the swap
> nudges the real figures **modestly better**; a full model refresh runs at the next
> online update (see `portfolio_optimiser/outputs/isa_5y/REFRESH_NOTE.md`).
...
| Blended ongoing charge (TER) | **0.38% / yr** (~£77 on £20,400) |
...
**Recommendation: build the Pie in §4.** Following the review you made two calls,
now baked in: **silver is dropped and replaced with water** in the rotating sleeve
(silver was a return-drag; water is a genuinely non-AI theme — see §7/§8), and the
**~34% Avantis weighting is kept** as a conscious choice. It matches the Balanced
risk level you chose, keeps a genuine growth engine while holding a **~16% genuine
crisis-hedge sleeve (gold + trend)** — plus ~10% inflation-sensitive, equity-like
infrastructure — and expresses your "interesting, not-yet-crowded" brief through
the rotating sleeve, without betting the portfolio on it. Note that "Balanced" here still means **~74% risk assets and no conventional bonds until the glidepath** (§10); the low 9.5% volatility relies on diversification that tends to compress in a genuine crisis.
...
| Themes | **Second-order**, undervalued, *not* the crowded trade | Fresh sleeve (grid, nuclear fuel-cycle, Japan value-up, water); silver, defence & uranium *miners* on the bench (drag or now-consensus). |
...
| Cost | Minimise vs Nutmeg | Blended ~0.38% vs Nutmeg ~0.95–1.0% all-in. |
...
| iShares Global Water | `IH2O.L` | Water infra/utilities | 5.0 | **Replaced silver after the review.** Water utilities and infrastructure — structural (scarcity, replacement of ageing networks), defensive, and **genuinely uncorrelated with the AI-capex trade** the rest of the sleeve leans into. Modelled ~6% compounded at ~15% vol — a real return-seeker, unlike the silver it replaced. |
...
> themes differently. The sleeve is a deliberate, mandate-driven overlay; it is not
> claimed to be return-optimal.

*(Silver `ISLN.L` — replaced by water this year — plus defence `NAVY.L` and uranium miners `URNP.L` remain on the bench for future rotations — see §8.)*
...
> the book**: `CUKX.L` (UK core), `AVEM.L` (EM core), `NUCG.L`, `IJPH.L`, and the new
> `IH2O.L` (water). Note `IH2O.L` is a **USD-quoted** LSE line (small FX effect on the
> quoted price) — a GBP-quoted water ETF is a fine alternative if you prefer.
...
> **`WIRE.L`** (grid), a **broad UK line** (`ISF.L`/`IUKD.L`), and confirm each is
> searchable and tradable in your Trading 212 ISA before funding.
...
2. **Risk from long history.** Covariance is estimated on ~17 years of monthly GBP total returns (from 2009), with young funds spliced onto longer-history proxies (e.g. the nuclear sleeve onto VanEck's 2007-vintage US analogue, grid onto the 2009 First Trust smart-grid index, water onto the 2007 S&P Global Water index fund).
...
- **Four themes — chosen for freshness, but read the honest labels.** Grid/electrification, the nuclear *fuel cycle*, Japan *value-up*, and (following the review) **water infrastructure**, each sized as a ~5% satellite. The honest caveats:
  ...
  - **Water is the deliberate counterweight** — a structural, defensive theme (scarcity, ageing-network replacement) whose drivers have **nothing to do with AI capex**, so it dilutes the sleeve's one-way-long-AI tilt. It replaced silver, which the model expected to compound at just ~0.8% (a return-drag, now benched — §8).

  Other on-brief, uncrowded ideas sit on the **bench** (agriculture/fertiliser, plus silver as pure inflation optionality) for future rotations.
...
| Silver `ISLN.L` · agriculture/fertiliser | Silver as pure inflation optionality (dropped this year); agriculture defensive/structural — natural rotations if a current theme gets hot. |
...
> **Two composition decisions — made after the review.** The analyst review surfaced
> two calls; both are now taken, resolved by a single swap:
> - **Silver (`ISLN.L`) dropped.** It was the lowest-return holding in the book
>   (~0.8% modelled compounded, ~28% vol) and diversified *worse* than the gold already
>   in the core; holding it cost on the order of **0.3–0.4%/yr** of expected growth. It
>   moves to the bench (available as pure inflation optionality in a future rotation).
> - **Water (`IH2O.L`) added in its place.** Grid + nuclear are ~10% net-long the
>   AI-power cycle; water is a **genuinely orthogonal** theme (its drivers are scarcity
>   and network replacement, not AI capex), so the sleeve is no longer one-way long the
>   same factor as the crowd — *and* it is a real return-seeker (~6% modelled compounded
>   at ~15% vol), not a drag.
>
> The headline numbers earlier in this note were modelled with silver in; water's higher
> return and lower vol make the true figures **modestly better** — a full refresh runs
> at the next online model update (see the build sheet and `REFRESH_NOTE.md`).
...
- **Theme concentration.** The rotating sleeve is higher-octane (the energy themes ~14–19% standalone volatility, water ~15%, Japan ~17%). Each is capped at ~5% for this reason; do not let a winner run far beyond that without understanding the added risk.
...
| iShares Global Water | `IH2O.L` | ~6.0% | ~15% | 0.65% | Rotating (replaced silver — see §8) |

*Water figures are the building-block estimate (developed-equity + a +1.0% water
premium, net of the 0.65% TER, ~15% vol from its water-index proxy); they will be
confirmed by the Monte-Carlo at the next online refresh. For reference, the silver
line it replaced modelled at just **0.8% compounded / 28.2% vol** — the
lowest-return, near-highest-vol line in the book and below the gold already in the
core, which is why the review flagged it and the account holder swapped it out.*
...
**On the theme covariance.** These modelled theme vols/correlations lean on proxy-spliced history (§5, §9) — mostly US-listed, differently-constructed analogues for new GBP/GBP-hedged UCITS — and the thematic return premia in `config/cma.toml` are deliberately uniform round numbers (+1.0% each for grid/nuclear/Japan/water) with no crowding haircut. They are a *transparent prior* so the sensitivity is auditable, not a precise forecast; the 20% sleeve is the least-certain part of the model.
